                ...
                switch (element) {
                    ...
                    case "Источник постоянного тока":
                        str += "<tr><td>ЭДС: <br><input type=\"text\" class_name='current_source' class='show_U_and_R' unit='e' onchange='validate_values(this)' value='1600'/></td>" +
                            "<td><br><select class='show_U_and_R' onchange='validate_values(this)'><option value='mili'>мВ</option><option value='deca'>В</option><option value='kilo'>кВ</option><option value='mega'>МВ</option></select></td></tr>"
                            + "<tr><td>Внутреннее сопротивление: <br><input type=\"text\" id=\"internal_resistance\" class_name='current_source' class='show_U_and_R' unit='r' onchange='validate_values(this)' value='500'>" + "</td>" +
                            "<td><br><select id=\"internal_resistance_unit\" class='show_U_and_R' onchange='validate_values(this)'><option value='mili'>мОм</option><option value='deca'>Ом</option><option value='kilo'>кОм</option><option value='mega'>МОм</option></select></td></tr>"
                            + "</table>" + "</div></td></tr>";
                        break;
    ...
    for (let i = 0; i < N * M; i++) {
        ...
        if (button !== null) {
            ...
            document.querySelectorAll('.show_U_and_R').forEach(input => {
                if (is_running === true) input.hidden = true;
            ...
            switch (element) {
                case "Вольтметр":
                    if(ELEMENT_CALCULATION.has(i)){
                        ...
                        replaceField(div, "<br><table>"
                            + "<tr><td><header style='font-size: larger'>Результат измерения</header><div class='show_U_and_R show_U'>" + "Напряжение на участке цепи: " + U + " В</td></tr>"
                            + "</table>");
                    }
                ...
                case "Амперметр":
                    if(ELEMENT_CALCULATION.has(i)) {
                        ...
                        replaceField(div, "<br><table>"
                            + "<tr><td><header style='font-size: larger'>Результат измерения</header><div class='show_U_and_R show_I'>" + "Сила тока на участке цепи: " + I.toFixed(5) + " А</td></tr>"
                            + "</table>");
                    }
                    break;
                case "Омметр":
...
function runChain(e) {
    ...
    if (e.getAttribute("is_running") === "false") {
        ...
        if (flagQ) {
            ...
            let voltmeterVoltage = parseFloat(document.querySelector('[class_name="current_source"]').value) - resultMap.get('Сила тока в цепи')[0] * (parseFloat(document.querySelector('#internal_resistance').value))
            let ammeterCurrent = voltmeterVoltage / (ammeter + parseFloat(document.querySelector('[class_name="resistor"]').value));

            document.querySelector('.show_U_and_R.show_U').innerHTML = `Напряжение на участке цепи: ${voltmeterVoltage.toFixed(3)} В`;
            document.querySelector('.show_U_and_R.show_I').innerHTML = `Сила тока на участке цепи: ${ammeterCurrent.toFixed(3)} А`;

            document.querySelector('.show_U_and_R.show_U').removeAttribute('hidden');
            document.querySelector('.show_U_and_R.show_I').removeAttribute('hidden');
        ...
        if (flagW) {
            ...
            let ammeterCurrent = resultMap.get('Сила тока в цепи')[0];
            let voltmeterVoltage = parseFloat(document.querySelector('[class_name="current_source"]').value) - resultMap.get('Сила тока в цепи')[0] * (parseFloat(document.querySelector('#internal_resistance').value)) - ammeterCurrent * ammeter;
        ...
        if (flagA) {
            ...
            let ammeterCurrent = resultMap.get('Сила тока в цепи')[0];
            let voltmeterVoltage = parseFloat(document.querySelector('[class_name="current_source"]').value) - resultMap.get('Сила тока в цепи')[0] * (parseFloat(document.querySelector('#internal_resistance').value)) - ammeter * resultMap.get('Сила тока в цепи')[0];
