...
function countSerialU(I, array){
    return I * countSerialR(cellArrayToNumber(array, "r")) / 2;
}
 ...
 */
function countFullCircuitI(R, e) {
    return (e / (R ));
}
/*
...
function countFullR(resultMap){
    ...
    let internalResistance = parseFloat(document.getElementById('internal_resistance').value);
    let unit = document.getElementById('internal_resistance_unit').value;

    // Преобразуем внутреннее сопротивление в единицы измерения "Ом"
    switch (unit) {
        case 'mili':
            internalResistance *= 0.001;
            break;
        case 'deca':
            // В этом случае сопротивление уже в Омах
            break;
        case 'kilo':
            internalResistance *= 1000;
            break;
        case 'mega':
            internalResistance *= 1000000;
            break;
        default:
            console.error("Unknown unit");
            break;
    }
    var finR = finalR+internalResistance
    resultMap.set('Полное сопротивление в цепи', [finR, 'Ом']);
}
...
function countFullI(resultMap){
    ...
    if (r !== null && e !== null) {
        let finalI = countFullCircuitI(finalR, e);
